...
def calculate_comparison_metrics(
    ...
    if not ground_truth and not hypothesis:
        ...
        try:
            ...
            results[wer_key] = measures.get('wer', 1.0) # Default to max error if key missing
            results[cer_key] = measures.get('cer', 1.0)
        except Exception as e:
...
def evaluate_document_image(
    ...
    ocr_text_raw = extract_text_from_image(generated_image_path, lang=ocr_lang, psm=ocr_psm)
    ...
    if verbose:
        ...
        print(gt_processed_strict[:500] + ("..." if len(gt_processed_strict) > 500 else ""))
        print("--- End Processed Ground Truth ---")
        print("\n--- Processed OCR Output Snippet (Strict) ---")
        print(ocr_processed_strict[:500] + ("..." if len(ocr_processed_strict) > 500 else ""))
    ...
    print(f"Calculated Strict Sequence Similarity (Full): {full_metrics.get('strict_sequence_similarity (Full)', 'N/A'):.4f}")
    print(f"Calculated Strict WER (Full): {full_metrics.get('strict_wer (Full)', 'N/A'):.4f}")
    print(f"Calculated Strict CER (Full): {full_metrics.get('strict_cer (Full)', 'N/A'):.4f}")
    ...
    if verbose and full_metrics.get('strict_wer (Full)', 0) > 0:
         ...
         try:
             ...
             detailed_measures = jiwer.compute_measures(gt_processed_strict, ocr_processed_strict)
    ...
    print(f"Calculated Strict Sequence Similarity (Truncated): {truncated_metrics.get('strict_sequence_similarity (Truncated)', 'N/A'):.4f}")
    print(f"Calculated Strict WER (Truncated): {truncated_metrics.get('strict_wer (Truncated)', 'N/A'):.4f}")
    print(f"Calculated Strict CER (Truncated): {truncated_metrics.get('strict_cer (Truncated)', 'N/A'):.4f}")
    ...
    if verbose and truncated_metrics.get('strict_wer (Truncated)', 0) > 0 and num_ocr_tokens > 0 and len(gt_tokens) >= num_ocr_tokens:
         ...
         try:
             ...
             detailed_measures_trunc = jiwer.compute_measures(gt_viz, ocr_processed_strict)
             print(jiwer.visualize_alignment(detailed_measures_trunc, show_measures=False))
             print("----------------------------------------------")
         except Exception as e:
             print(f"Could not generate detailed alignment for truncated comparison: {e}")
    ...
    results = {
        ...
        'ocr_success': ocr_text_raw is not None, # True if OCR didn't have critical failure
        'ocr_found_text': bool(ocr_text_raw) # True if OCR returned non-empty string
    }
...
if __name__ == "__main__":
    ...
    if evaluation_results:
        ...
        print(f"Strict Sequence Similarity (0-1, higher=better): {sim_full:.4f}" if sim_full is not None else "Strict Sequence Similarity: N/A")
        print(f"Strict Word Error Rate (WER) (0+, lower=better) : {wer_full:.4f}" if wer_full is not None else "Strict WER: N/A")
        print(f"Strict Character Error Rate (CER) (0+, lower=better): {cer_full:.4f}" if cer_full is not None else "Strict CER: N/A")

        print("\n--- Truncated Comparison Metrics ---")
        print(f"(Comparing OCR output vs. Ground Truth truncated to {evaluation_results.get('ocr_tokens', 'N/A')} tokens)")
        ...
        print(f"Strict Sequence Similarity (0-1, higher=better): {sim_trunc:.4f}" if sim_trunc is not None else "Strict Sequence Similarity (Truncated): N/A")
        print(f"Strict Word Error Rate (WER) (0+, lower=better) : {wer_trunc:.4f}" if wer_trunc is not None else "Strict WER (Truncated): N/A")
        print(f"Strict Character Error Rate (CER) (0+, lower=better): {cer_trunc:.4f}" if cer_trunc is not None else "Strict CER (Truncated): N/A")

        print("\n--- Text Statistics ---")
        print(f"Ground Truth Length (raw chars) : {evaluation_results['ground_truth_len_raw_chars']}")
        print(f"OCR Output Length (raw chars)   : {evaluation_results['ocr_text_len_raw_chars']}")
        print(f"Ground Truth Tokens (processed) : {evaluation_results['ground_truth_tokens']}")
        ...
        print(f"OCR Found Any Text              : {evaluation_results['ocr_found_text']}")
    else:
