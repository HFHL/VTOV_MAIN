...
def get_text_image_pairs(path: str) -> List[Tuple[str, str]]:
    ...
    if path.endswith(".json") and os.path.isfile(path):
        base_dir = os.path.dirname(path) # Paths in JSON might be relative to JSON location
        try:
            ...
            if isinstance(data, list):
                for item in data:
                    if isinstance(item, dict) and "local_text_path" in item and "local_image_path" in item:
                        # Resolve paths relative to the JSON file's directory
                        text_path = os.path.join(base_dir, item["local_text_path"])
                        image_path = os.path.join(base_dir, item["local_image_path"])
                        if os.path.exists(text_path) and os.path.exists(image_path):
                            pairs.append((text_path, image_path))
                        else:
                             print(f"Warning: Skipping pair from JSON. File not found: {text_path if not os.path.exists(text_path) else image_path}")

            else:
                 print(f"Warning: JSON file '{path}' does not contain a list of objects with 'text' and 'image' keys.")
            return pairs
    ...
    text_files = sorted([f for f in files if f.endswith(".txt")]) # Sort for consistency

    # For each text file, look for a corresponding image file
    for text_file in text_files:
        base_name = os.path.splitext(text_file)[0] # General way to remove extension
...
def bootstrap_statistics(
    ...
    """
    if not values:
        print(f"Warning: Cannot calculate bootstrap statistics for '{metric_name}' - no valid data points.")
        return None
    if len(values) == 1:
         print(f"Warning: Only one data point for '{metric_name}'. Reporting mean, std_dev=0, CI=mean.")
    ...
    lower_idx = max(0, int(0.025 * n_bootstrap) -1) # -1 because of 0-based index
    ...
    if n_bootstrap < 40: # Arbitrary threshold, adjust if needed
        print(f"Warning: Low number of bootstrap samples ({n_bootstrap}) for '{metric_name}'. CI might be unreliable.")
...
def evaluate_pairs(
    ...
    for i, (text_path, image_path) in enumerate(pairs):
        ...
        try:
            ...
            similarities.append(eval_metrics['strict_sequence_similarity (Full)'])
            similarities_truncated.append(eval_metrics['strict_sequence_similarity (Truncated)'])
            ...
            print(f"Error during evaluation process for pair {text_path} - {image_path}: {e}")
...
def main():
    ...
    parser.add_argument(
        ...
        help="Path to directory containing pairs of text and image "
    ...
    def print_metric_stats(metric_name: str, metric_stats: Optional[Dict[str, float]]):
        ...
        if metric_stats:
            ...
            print(f"  95% CI (Bootstrap): [{metric_stats['ci_lower']:.4f}, {metric_stats['ci_upper']:.4f}]")
        else:
            print("  (No valid data to calculate statistics)")

    print_metric_stats("Sequence Similarity (Higher=Better)", stats["strict_sequence_similarity (Full)"])
    print_metric_stats("Sequence Similarity (Higher=Better)", stats["strict_sequence_similarity (Truncated)"])
